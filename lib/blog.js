...
/**
 * Every post: pinned ones first, then newest first.
 * `pinned: true` in the frontmatter holds a post at the top regardless of how
 * many newer posts arrive after it. Dates still order everything else, and
 * ties between two pinned posts fall back to the date, so the order stays
 * deterministic rather than depending on how readdir happened to list them.
 */
