...
/**
 * Every post: hand-placed ones first in `order`, then the rest oldest first.
 *
 * A post with `order: 1` holds the top of the index however many posts arrive
 * after it — the date no longer decides its position, which is the point.
 * Posts without an `order` fall in behind in date order, earliest first, so
 * the index reads as a sequence rather than a feed.
 *
